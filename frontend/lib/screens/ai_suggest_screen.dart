...
class _AiSuggestScreenState extends State<AiSuggestScreen> {
              ...
              'messages': [
                {
                  'role': 'system',
                  'content':
                      'You are a helpful fashion stylist. Return a concise outfit suggestion with an outfit name, specific items, and a short style tip. Consider the user\'s gender when suggesting outfits.',
                },
  ...
  String _buildLocalFallbackSuggestion() {
    ...
    if (state.isMaleWardrobe) {
      final summer = pickByCategory('Summer');
      final winter = pickByCategory('Winter');
      final shoes = pickByCategory('Footwear');
      fallbackItems = [summer, winter, shoes]
      ...
      final shoes = pickByCategory('Footwear');
      fallbackItems = [top, bottom, outerwear, shoes]
          .where((item) => item.isNotEmpty)
          .toList();
    }
