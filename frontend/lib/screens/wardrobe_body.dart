...
class _WardrobeBodyState extends State<WardrobeBody> {
  ...
  WardrobeCategory get _currentCategory {
    final cats = context.read<AppState>().isMaleWardrobe
  ...
  List<ClothingItem> _filteredItems(List<ClothingItem> items) {
    ...
    if (_filterColor != null) {
      filtered = filtered
          .where((i) => i.color == _filterColor)
          .toList();
    }
    if (_filterPieces != null) {
      final pieces = int.tryParse(
            _filterPieces!.replaceAll(RegExp(r'[^0-9]'), ''),
          ) ??
          0;
      filtered = filtered
          .where((i) => i.dressPieces == pieces)
          .toList();
    }
    return filtered;
  }
  ...
  Widget _buildMainCategoryChips() {
    final cats = context.watch<AppState>().isMaleWardrobe
        ? DummyData.maleCategories
        : DummyData.categories;
    return SizedBox(
      height: 44,
          ...
          child: ListView(
            ...
            children: [
              ...
              _buildFilterOption(
                'All',
                _filterPieces == null,
                () => setState(() => _filterPieces = null),
              ),
              ...DummyData.dressPiecesOptions.map((piece) {
                return _buildFilterOption(
                  piece,
                  _filterPieces == piece,
                  () => setState(() => _filterPieces = piece),
                );
              }),
            ],
          ),
